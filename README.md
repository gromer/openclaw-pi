...
Raspberry Pi 5 running Raspberry Pi OS, checks out an immutable repository ref,
and runs the selected validation or provisioning target.
...
Replace `OWNER` and `v1.0.0` below with the actual repository owner and published
release. Download the script and checksum as separate files—do not pipe the
network response directly into a root shell:

```sh
curl -fsSLO https://github.com/OWNER/openclaw-pi/releases/download/v1.0.0/bootstrap.sh
curl -fsSLO https://github.com/OWNER/openclaw-pi/releases/download/v1.0.0/bootstrap.sh.sha256
sha256sum --check bootstrap.sh.sha256
less bootstrap.sh
sudo OPENCLAW_PI_REPO_URL=https://github.com/OWNER/openclaw-pi.git \
  OPENCLAW_PI_REF=v1.0.0 \
  OPENCLAW_PI_INVENTORY=/root/openclaw-inventory/hosts.yml sh bootstrap.sh
```

`OPENCLAW_PI_REF` is required and mutable names such as `main`, `master`, `latest`,
and `HEAD` are rejected. A signed release tag is recommended; a full commit SHA
also works. `OPENCLAW_PI_DEST` optionally changes the checkout location from
`/opt/openclaw-pi`. The script is safe to rerun and updates an existing checkout
before detaching it at the requested immutable ref.
...
```sh
sudo OPENCLAW_PI_REPO_URL=https://github.com/OWNER/openclaw-pi.git \
  OPENCLAW_PI_REF=v1.0.0 sh bootstrap.sh --validate-only
...
For a private repository, preconfigure root's SSH `known_hosts` and a read-only
deploy key, then use an `ssh://` or `git@host:path` repository URL. Never put
tokens, SOPS values, Restic passwords, or age identities in command-line flags or
environment values passed to the bootstrap script. See
