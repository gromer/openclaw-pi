...
Release assets are generated by GitHub Actions. Replace `OWNER`, verify the
release/tag and checksum through an independent trusted channel, then:
...
Stage the ignored production inventory (and SOPS identity when the Pi is also the
controller) securely before provisioning; releases contain no machine values.
For a private repository, preconfigure root's SSH host key and deploy key, then
use an `ssh://` or `git@host:path` URL. Do not put tokens or secrets in arguments.
`--validate-only`, `--dry-run`, and `--help` are supported. The documented flow
does not pipe network content into a privileged shell.
