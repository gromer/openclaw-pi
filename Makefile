...
ANSIBLE_ARGS ?=
.DEFAULT_GOAL := help
.PHONY: help bootstrap deps preflight provision validate check syntax lint diff verify backup restore secrets-init secrets-edit secrets-check backup-check compose-check release-test actions-policy-check
help: ## Show targets
...
validate: syntax compose-check secrets-check actions-policy-check release-test ## Run credential-free structural validation
